...
class ExploreTabRepositoryImpl implements ExploreTabRepository{
  ...
  ExploreTabRepositoryImpl({required this.exploreTabRemoteDataSource,required this.exploreTabLocalDataSource});
  @override
  Future<BaseResponse<List<SubjectEntity>>> getAllSubjects(String token) async{
    BaseResponse<String> tokenFetching=await exploreTabLocalDataSource.getToken(token: token);
    BaseResponse<List<SubjectDTO>> subjectsResponse=await exploreTabRemoteDataSource.getAllSubjects(tokenFetching.toString());
    switch(subjectsResponse){

      case SuccessResponse<List<SubjectDTO>>():
        List<SubjectDTO> subjectsDTO=subjectsResponse.data;
        List<SubjectEntity> subjects=subjectsDTO.map((e) => e.toDomain(),).toList();
        return SuccessResponse<List<SubjectEntity>>(data: subjects);
      case ErrorResponse<List<SubjectDTO>>():
        return ErrorResponse<List<SubjectEntity>>(error: subjectsResponse.error);
    }
  }
